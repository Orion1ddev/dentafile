...
const Dashboard = () => {
  ...
  const features = [{
    Icon: () => {
      ...
      return hour >= 6 && hour < 18 ? <Sun className="h-12 w-12" /> : <Moon className="h-12 w-12" />;
    },
    name: t('welcome'),
    description: "",
    href: "#",
    cta: "",
    background: <WelcomeCard userProfile={userProfile} appointmentCount={todayAppointments?.length || 0} pinnedPatientsCount={pinnedPatients?.length || 0} />,
  ...
  return <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="max-w-[2000px] p-4 flex-1 flex items-center py-[16px] px-[104px] my-0 mx-0">
        <div className="space-y-8 w-full">
          <BentoGrid className="lg:grid-rows-3">
            {features.map(feature => <BentoCard key={feature.name || 'welcome'} {...feature} />)}
          </BentoGrid>
        </div>
      </main>
      <NavMenu />
